...
context_length = 32
train_iter, vocab = get_data_iter_for_bert(context_length)
...
def get_bert_batch_loss(model, loss, vocab_size, tokens_X,
...
def train_bert_model(model, train_iter, loss, lr, num_epochs):
    if num_devices > 1:
    ...
    while epoch < num_epochs:
        ...
        for tokens_X, segments_X, attn_mask_X, pred_positions_X,\
            ...
            mlm_loss, nsp_loss, batch_loss = get_bert_batch_loss(
                model, loss, vocab_size, tokens_X, segments_X, attn_mask_X,
